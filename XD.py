#!/usr/bin/env python3
import subprocess, sys, importlib, os, time, random, shutil, re, json, base64, hashlib, sqlite3, csv, webbrowser
...
import requests, whois, phonenumbers
...
def save_config(databases, saved_key=None):
    data = {"databases": databases}
    if saved_key:
        data["saved_key"] = saved_key
    try:
...
def load_config():
    try:
        with open(CONFIG_FILE, 'r') as f:
            data = json.load(f)
            return data.get("databases", []), data.get("saved_key", "")
    except:
        return [], ""
        ...
        for y in range(rows):
            ...
            for x in range(cols):
                ...
                if drops[y][x] > 0:
                    ...
                    line += " "
            print(line)
        sys.stdout.write(f"{HOME}{RED_BRIGHT}\033[{prompt_y+1};{prompt_x}H{prompt}{RESET}")
        sys.stdout.flush()
        time.sleep(0.04)
...
def key_screen():
    clear_screen()
    global VERSION_MODE
    saved_dbs, saved_key = load_config()
    if saved_key:
    ...
    for row in range(box_height):
        sys.stdout.write(f"\033[{box_y + row};{box_x}H")
    ...
    sys.stdout.write(f"\033[{prompt_y};{prompt_x}H{RED_BOLD}Введите ключ: {RESET}")
    sys.stdout.flush()
    attempt = 0
    while attempt < 3:
        key = input("").strip()
    ...
    try:
        if sys.platform.startswith('win'):
            ...
            if shutil.which('ffplay'):
                ...
                return "ffplay не найден. Установите: pkg install ffplay"
...
def print_header():
    ...
    lines.append(f"{RED_BOLD}   FPYTHONSOCIAL v33.0     {RED_DIM}от {AUTHOR}{RESET}")
    lines.append(f"{RED_BOLD}{'═'*60}{RESET}")
