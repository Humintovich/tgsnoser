...
def typewriter_lines(lines, delay=0.005):
    for line in lines:
...
def key_screen():
    ...
    lines.append(f"{RED_DIM}   BASIC2025 или PREMIUM2025{RESET}")
    for line in lines:
        spaces = (cols - len(line)) // 2
        if spaces < 0:
            spaces = 0
        typewriter(" " * spaces + line, 0.005)
    ...
    while attempt < 3:
        ...
        if key == BASIC_KEY:
            ...
            save_config(config | {"saved_key": key})
            time.sleep(1)
            return True
